...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($formAction === 'delete') {
        ...
        if ($docId > 0) {
            ...
            // Delete document
            $stmtDel = $pdo->prepare("DELETE FROM documents WHERE id = :id AND direction = 'Outgoing'");
            $stmtDel->execute([':id' => $docId]);
            setFlash('success', 'Outgoing document record deleted successfully.');
        }
        header("Location: outgoing.php");
        exit();
    }
    ...
    if ($formAction === 'bulk_delete') {
        ...
        if (is_array($selectedIds) && !empty($selectedIds)) {
            ...
            if (!empty($ids)) {
                ...
                $stmtDel = $pdo->prepare("DELETE FROM documents WHERE id IN ($inQuery) AND direction = 'Outgoing'");
                $stmtDel->execute($ids);
                setFlash('success', count($ids) . ' outgoing document record(s) deleted successfully.');
            }
        }
        header("Location: outgoing.php");
        exit();
    ...
    if ($formAction === 'save') {
        ...
        try {
            ...
            if ($docId > 0) {
                ...
                $stmtUpdate = $pdo->prepare("
                    UPDATE documents SET 
                        ...
                        time_log = :time_log,
                        remarks = :remarks
                    WHERE id = :id AND direction = 'Outgoing'
                ");
                $stmtUpdate->execute([
                    ':ref' => $referenceNumber,
                    ':title' => $documentTitle,
                ...
                $savedDocId = $docId;
                setFlash('success', 'Outgoing document updated successfully.');
            } else {
                // Insert New Document
                $stmtInsert = $pdo->prepare("
                    INSERT INTO documents 
                        (reference_number, document_title, category_id, document_type_id, direction, recipient_office, document_date, time_log, remarks, encoded_by)
                    VALUES 
                        (:ref, :title, :cat, :type, 'Outgoing', :recipient, :doc_date, :time_log, :remarks, :encoded_by)
                ");
                $stmtInsert->execute([
                    ':ref' => $referenceNumber,
                    ':title' => $documentTitle,
                ...
                setFlash('success', 'Outgoing document entry logged successfully.');
            }
            ...
            // Process Selected Files from Incoming Log Section
            $selectedIncomingAtts = $_POST['incoming_attachments'] ?? [];
            if (is_array($selectedIncomingAtts) && !empty($selectedIncomingAtts)) {
                $stmtGetInc = $pdo->prepare("
                    SELECT da.*, ai.item_name, d.reference_number, d.document_title
                    FROM document_attachments da
                    JOIN documents d ON da.document_id = d.id
                    LEFT JOIN attachment_items ai ON da.attachment_item_id = ai.id
                    WHERE da.id = :att_id AND d.direction = 'Incoming'
                ");
                $stmtInsertAtt = $pdo->prepare("
                    INSERT INTO document_attachments (document_id, attachment_item_id, custom_item_name, file_path) 
                    VALUES (:doc_id, :item_id, :cname, :file_path)
                ");
                foreach ($selectedIncomingAtts as $incAttId) {
                    $incAttId = intval($incAttId);
                    if ($incAttId > 0) {
                        $stmtGetInc->execute([':att_id' => $incAttId]);
                        $incAtt = $stmtGetInc->fetch();
                        if ($incAtt) {
                            $baseName = $incAtt['item_name'] ?: ($incAtt['custom_item_name'] ?: basename($incAtt['file_path']));
                            $refText = !empty($incAtt['reference_number']) ? ('Ref: ' . $incAtt['reference_number']) : ('Doc #' . $incAtt['document_id']);
                            $customLabel = $baseName . ' (From Incoming ' . $refText . ')';
                            $stmtInsertAtt->execute([
                                ':doc_id' => $savedDocId,
                                ':item_id' => $incAtt['attachment_item_id'],
                                ':cname' => $customLabel,
                                ':file_path' => $incAtt['file_path']
                            ]);
                        }
                    }
                }
...
// Build Query
$whereClause = ["d.direction = 'Outgoing'"];
$params = [];

if (!empty($search)) {
    $whereClause[] = "(d.reference_number LIKE :search1 OR d.document_title LIKE :search2 OR d.recipient_office LIKE :search3)";
...
// Fetch uploaded attachments from Incoming documents for direct selection
$stmtIncAtt = $pdo->query("
    SELECT da.id AS attachment_id, da.file_path, da.custom_item_name, da.attachment_item_id,
           ai.item_name, d.id AS doc_id, d.reference_number, d.document_title, d.document_date
    FROM document_attachments da
    JOIN documents d ON da.document_id = d.id
    LEFT JOIN attachment_items ai ON da.attachment_item_id = ai.id
    WHERE d.direction = 'Incoming'
    ORDER BY d.id DESC, da.id ASC
");
$incomingAttachments = $stmtIncAtt->fetchAll();
...
if ($viewDocId > 0) {
    $stmtV = $pdo->prepare("
        ...
        JOIN document_types dt ON d.document_type_id = dt.id
        LEFT JOIN users u ON d.encoded_by = u.id
        WHERE d.id = :id AND d.direction = 'Outgoing'
    ");
...
<div class="container-fluid px-4 py-2">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 pb-2 border-bottom">
        <div>
            <h3 class="fw-bold mb-1" style="color: #b45309;">
                <i class="bi bi-box-arrow-up me-2 text-warning"></i> Outgoing Document Logbook
            </h3>
            <p class="text-muted mb-0 small">Repository for all outgoing files, communications, and documents dispatched to recipient offices.</p>
        </div>
        <?php if (!hasRole('viewer')): ?>
        <div class="mt-3 mt-md-0">
            <button type="button" class="btn btn-accent-custom shadow-sm" data-bs-toggle="modal" data-bs-target="#addDocumentModal">
                <i class="bi bi-plus-lg me-1"></i> Add Outgoing Entry
            </button>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="card card-custom mb-4">
        <div class="card-body p-3">
            <form method="GET" action="outgoing.php" class="row g-2 align-items-end">
                <div class="col-12 col-md-3">
                    <label class="form-label small fw-semibold mb-1">Search Keyword</label>
                    <input type="text" name="search" class="form-control form-control-sm" placeholder="Ref #, Title, or Recipient..." value="<?= sanitize($search) ?>">
                </div>
                ...
                <div class="col-12 col-md-1 d-flex gap-1">
                    <button type="submit" class="btn btn-sm btn-accent-custom w-100" title="Apply Filter">
                        <i class="bi bi-search"></i>
                    </button>
                    <a href="outgoing.php" class="btn btn-sm btn-outline-secondary" title="Reset Filters">
    ...
    <form method="POST" action="outgoing.php" id="bulkDeleteForm">
        <input type="hidden" name="form_action" value="bulk_delete">
        <div class="card card-custom">
            <div class="card-header card-header-custom d-flex justify-content-between align-items-center">
                <h5 class="card-title fw-bold mb-0 text-dark">
                    <i class="bi bi-list-ul me-2 text-warning"></i> Outgoing Entries Log
                </h5>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td colspan="<?= hasRole('admin') ? '10' : '9' ?>" class="text-center py-5 text-muted">
                                    ...
                                    No outgoing document logs matched your criteria.
                                </td>
                            </tr>
                            <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <?php endif; ?>
                                    <td class="fw-bold text-warning font-monospace" style="font-size:0.85rem; color:#b45309 !important;"><?= sanitize($doc['reference_number'] ?: '—') ?></td>
                                    ...
                                    <td class="small text-nowrap text-muted"><i class="bi bi-clock me-1 text-warning opacity-75"></i><?= !empty($doc['time_log']) ? date('h:i A', strtotime($doc['time_log'])) : (!empty($doc['created_at']) ? date('h:i A', strtotime($doc['created_at'])) : '—') ?></td>
                                    <td class="small text-muted"><?= sanitize($doc['encoder_name'] ?: 'System') ?></td>
                                    <td class="text-end text-nowrap">
                                        <a href="outgoing.php?view=<?= $doc['id'] ?>" class="btn btn-sm btn-outline-primary-custom py-1 px-2" title="View Details">
...
<!-- Modal: Add / Edit Outgoing Document -->
<div class="modal fade" id="addDocumentModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <form method="POST" action="outgoing.php" enctype="multipart/form-data" class="modal-content border-0 shadow-lg">
            <div class="modal-header text-dark" style="background: linear-gradient(135deg, #edac36 0%, #d97706 100%); border-bottom: 3px solid var(--primary-color);">
                <h5 class="modal-title fw-bold" id="modalTitle">
                    <i class="bi bi-box-arrow-up me-2"></i> Log New Outgoing Document
                </h5>
            ...
            <div class="modal-body p-4">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="reference_number" name="reference_number" placeholder="e.g. OUT-2026-001 (optional)">
                    ...
                    <!-- Recipient Office -->
                    <div class="col-12">
                        <label for="recipient_office" class="form-label">Recipient Office / Entity</label>
                        <input type="text" class="form-control" id="recipient_office" name="recipient_office" placeholder="e.g. Schools Division Office / Budget Unit">
                    </div>
                    ...
                    <!-- Attach Existing Files from Incoming Log Section -->
                    <div class="col-12">
                        <div class="card border border-warning-subtle shadow-sm mb-2" style="background-color: #fffdf5;">
                            <div class="card-header py-2 bg-light d-flex justify-content-between align-items-center">
                                <span class="fw-bold small" style="color: #b45309;">
                                    <i class="bi bi-box-arrow-in-down me-1 text-warning"></i> Select Existing Files from Incoming Log
                                </span>
                                <span class="badge bg-secondary font-monospace"><?= count($incomingAttachments) ?> files available</span>
                            </div>
                            <div class="card-body p-3">
                                <?php if (empty($incomingAttachments)): ?>
                                    <div class="text-muted small italic">No uploaded attachment files currently exist in the Incoming Logbook.</div>
                                <?php else: ?>
                                    <div class="input-group input-group-sm mb-2">
                                        <span class="input-group-text bg-white"><i class="bi bi-search text-muted"></i></span>
                                        <input type="text" id="incoming_att_search" class="form-control form-control-sm" placeholder="Filter incoming files by Ref #, Title, or Filename...">
                                    </div>
                                    <div class="incoming-att-list p-2 bg-white rounded border" style="max-height: 180px; overflow-y: auto;">
                                        <?php foreach ($incomingAttachments as $incFile): 
                                            $fileName = $incFile['item_name'] ?: ($incFile['custom_item_name'] ?: basename($incFile['file_path']));
                                            $refLabel = !empty($incFile['reference_number']) ? $incFile['reference_number'] : ('Doc #' . $incFile['doc_id']);
                                            $searchKey = strtolower($refLabel . ' ' . $incFile['document_title'] . ' ' . $fileName);
                                        ?>
                                        <div class="d-flex align-items-center justify-content-between py-1 px-2 border-bottom incoming-att-item" data-search="<?= sanitize($searchKey) ?>">
                                            <div class="form-check me-2 mb-0 d-flex align-items-center text-truncate" style="max-width: 85%;">
                                                <input class="form-check-input me-2 flex-shrink-0" type="checkbox" name="incoming_attachments[]" value="<?= $incFile['attachment_id'] ?>" id="inc_att_<?= $incFile['attachment_id'] ?>">
                                                <label class="form-check-label text-dark small text-truncate" style="cursor: pointer;" for="inc_att_<?= $incFile['attachment_id'] ?>">
                                                    <span class="badge bg-warning text-dark font-monospace me-1"><?= sanitize($refLabel) ?></span>
                                                    <span class="fw-semibold me-1"><?= sanitize($fileName) ?></span>
                                                    <span class="text-muted small opacity-75">— <?= sanitize($incFile['document_title']) ?></span>
                                                </label>
                                            </div>
                                            <a href="<?= sanitize($incFile['file_path']) ?>" target="_blank" class="btn btn-sm btn-link text-decoration-none py-0 px-1 text-muted" title="View / Preview File">
            ...
            <div class="modal-footer bg-light">
                ...
                <button type="submit" class="btn btn-accent-custom px-4"><i class="bi bi-save me-1"></i> Save Outgoing Entry</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal fade show d-block" id="viewDocumentModal" tabindex="-1" style="background: rgba(0,0,0,0.5);">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header text-dark" style="background: linear-gradient(135deg, #edac36 0%, #d97706 100%);">
                <h5 class="modal-title fw-bold">
                    <i class="bi bi-file-earmark-text me-2 text-primary"></i> Outgoing Entry Details
                </h5>
                <a href="outgoing.php" class="btn-close"></a>
            </div>
            <div class="modal-body p-4">
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        <div class="text-muted small">Reference Number</div>
                        <div class="fs-5 fw-bold text-warning" style="color: #b45309 !important;"><?= sanitize($viewDoc['reference_number'] ?: 'N/A') ?></div>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-6">
                        <div class="text-muted small">Recipient Office</div>
                        <div class="fw-semibold text-dark"><?= sanitize($viewDoc['recipient_office'] ?: 'N/A') ?></div>
                    </div>
                    <div class="col-md-6">
                ...
                <?php else: ?>
                    <div class="list-group">
                        ...
                        <div class="list-group-item d-flex justify-content-between align-items-center py-2 attachment-row-<?= $att['id'] ?>">
                            <div>
                                <i class="bi bi-file-earmark-arrow-down fs-5 text-warning me-2"></i>
                                <span class="fw-semibold text-dark small">
                                    <?= sanitize($att['item_name'] ?: $att['custom_item_name'] ?: 'Attached File') ?>
                                </span>
                            </div>
                            <div class="d-flex gap-1 align-items-center">
                                <a href="<?= sanitize($att['file_path']) ?>" target="_blank" download class="btn btn-sm btn-outline-warning text-dark fw-semibold">
            ...
            <div class="modal-footer bg-light">
                <a href="outgoing.php" class="btn btn-secondary">Close</a>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    editBtns.forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil me-2"></i> Edit Outgoing Entry';
    ...
    // Filter Incoming Attachments List in Real-Time
    const incSearchInput = document.getElementById('incoming_att_search');
    if (incSearchInput) {
        incSearchInput.addEventListener('input', function () {
            const query = this.value.toLowerCase().trim();
            const items = document.querySelectorAll('.incoming-att-item');
            items.forEach(item => {
